<template>
  <div>
      ...
      <el-col :span="14">
        <el-card>
          ...
          <el-table :data="sprints" size="small" highlight-current-row @current-change="selectSprint">
            <el-table-column prop="name" label="迭代" />
    ...
    <el-dialog v-model="linkVisible" title="拉入需求" width="480px">
      <el-select v-model="linkStoryIds" multiple style="width: 100%" placeholder="选择要拉入的需求">
        <el-option v-for="s in storyOptions" :key="s.id" :label="`#${s.id} ${s.title}`" :value="s.id" />
      </el-select>
      <template #footer>
        <el-button @click="linkVisible = false">取消</el-button>
        <el-button type="primary" @click="linkStories">确定</el-button>
      </template>
    </el-dialog>
...
async function linkStories() {
  ...
  await sprintApi.linkStories(currentSprint.value.id, linkStoryIds.value)
